...
{
    fprintf(stderr, "Usage: %s <t> <k> <v> <method> [output_folder] [--balanced]\n", prog);
    fprintf(stderr, "  method: random | position | full\n");
    fprintf(stderr, "  --balanced: optional, balances symbol frequency in generated rows\n");
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(method, "random") == 0) {
        ...
        result = balanced ? ca_init_rotation_position_balanced(ca) : ca_init_rotation_position(ca);
    } else if (strcmp(method, "full") == 0) {
        result = balanced ? ca_init_rotation_full_balanced(ca) : ca_init_rotation_full(ca);
    } else {
    ...
    char comment[96];
    snprintf(comment, sizeof(comment), "creation method: %s, balanced: %s", method, balanced ? "yes" : "no");
