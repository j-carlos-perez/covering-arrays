...
static int find_best_values_for_tcolumns(covering_array_t *ca, const ca_affected_t *pre,
                                          int **IToC, int row_idx, uint16_t change_set_idx,
                                          int *out_best_values, ssize_t *out_delta)
{
    if (ca == NULL || pre == NULL || IToC == NULL || out_best_values == NULL || out_delta == NULL) {
        return -1;
    }
...
int main(int argc, char *argv[])
{
    if (argc != 3) {
        ...
        fprintf(stderr, "  Load a covering array from file and find best values for t-columns\n");
        return 1;
    }
    ...
    if (ca_save(output_folder, ca, "Optimized covering array (t-columns)", missing) == 0) {
