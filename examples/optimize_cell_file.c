...
static int find_best_value_for_cell(covering_array_t *ca,
                                    const ca_affected_t *pre, int **IToC,
                                    int row_idx, int col_idx,
                                    int *out_best_value, ssize_t *out_delta) {
  if (ca == NULL || pre == NULL || IToC == NULL || out_best_value == NULL ||
      out_delta == NULL) {
    return -1;
  }
  ...
  for (int candidate_val = 0; candidate_val < ca->v; candidate_val++) {
    ...
    ssize_t delta =
        ca_compute_cell_delta(ca, pre, IToC, row_idx, col_idx, candidate_val);
...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    ...
    fprintf(stderr, "  Load a covering array from file and find best value for "
                    "a random cell\n");
    return 1;
  }
  ...
  int best_value;
  ssize_t delta;
  if (find_best_value_for_cell(ca, pre, IToC, row_idx, col_idx, &best_value,
                               &delta) != 0) {
  ...
  if (ca_save(output_folder, ca, "Optimized covering array") == 0) {
