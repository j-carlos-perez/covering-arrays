...
{
    fprintf(stderr, "USAGE: %s -PATH 'out_folder' -FILE 'partialCA' -ROWIN 'int > 0' -ROWOUT 'int > 0' [-EXCLUDE 'int']\n", name);
}
...
int evaluate(int selRow, int nocovered)
{
    ...
    for (i = 0; i < nocovered; ++i) {
        for (j = 0; j < ROWOUT; ++j) {
            ...
            flag = 1;
            for (h = 0; h < t; ++h) {
                if (current[j][IToC[M[idx[i]][0]][h]] != alph[h] && current[j][IToC[M[idx[i]][0]][h]] != v) {
                    flag = 0;
                    break;
                }
            }
