...
ca_affected_t *precompute_create(size_t k, size_t t)
{
    ...
    if (R > UINT16_MAX) {
        fprintf(stderr, "Error: C(k,t) = %zu exceeds uint16_t max (%u). "
    ...
    if (total_memory > MAX_MEMORY_BYTES) {
        fprintf(stderr, "Error: total memory required (~%.1f GB) exceeds limit of %d GB\n",
    ...
    if (entries_per_col > UINT16_MAX) {
        fprintf(stderr, "Error: C(k-1,t-1) = %zu exceeds uint16_t max (%u). "
    ...
    size_t col_mapping_memory = (k + 1) * sizeof(size_t) + k * entries_per_col * sizeof(uint16_t);
    if (total_memory + col_mapping_memory > MAX_MEMORY_BYTES) {
        fprintf(stderr, "Error: total memory required (~%.1f GB) exceeds limit of %d GB\n",
                (double)(total_memory + col_mapping_memory) / (1024.0 * 1024.0 * 1024.0), MAX_MEMORY_GB);
        free(pre->indices);
        free(pre);
        return NULL;
    }
...
const uint16_t *precompute_get_affected(const ca_affected_t *pre, size_t change_set_idx)
{
    if (pre == NULL || change_set_idx >= pre->change_sets) return NULL;
...
const uint16_t *precompute_get_col_affected(const ca_affected_t *pre, size_t column)
{
    if (pre == NULL || column >= pre->k) return NULL;
...
size_t precompute_get_col_affected_count(const ca_affected_t *pre)
{
    ...
    return pre->col_offsets[pre->k] / pre->k;  // or: pre->col_offsets[column+1] - pre->col_offsets[column]
}